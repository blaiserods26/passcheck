...
function showText(divlayer, link) {
    if (
    document.getElementById(divlayer).style
        .display == "none"
    ) {
...
function toWords(number) {
    ...
    if (number < 120) {
    return (
        getNumberWords(number, true) + " seconds"
    );
    }
    var hour = 60 * 60;
    if (number < hour) {
    minutes = number / 60;
    return (
        getNumberWords(minutes, true) + " minutes"
    );
    }
    var day = hour * 24;
    if (number < 2 * day) {
    ...
    return (
        getNumberWords(centuries) + " centuries"
    );
    }
    years = number / year;
    return getNumberWords(years) + " years";
...
function getNumberWords(number, twoDP) {
    ...
    number =
    Math.round(number * decimalPoint) /
    decimalPoint;
...
function showHasChars(typeofchar, clear) {
    if (
    document.getElementById(typeofchar + "_count")
    ) {
    thiselem = document.getElementById(
        typeofchar + "_count"
    );
    } else {
    return false;
...
function displayWeakExplanation(
    matchSequence,
    strength
) {
...
function showHasChars(typeofchar, clear) {
    ...
    if (
        thiselem.className.indexOf(
        "char_type_valid"
        ) == -1
    ) {
        thiselem.className =
        thiselem.className + " char_type_valid";
    }
    }
}

function display_result(){
    document.getElementById("initial_estimate").style.display = "none";
    document.getElementById("first_estimate").style.display = "block";
}

function reset(){
    // Select the password input field and the element to hide
    const passwordInput = document.querySelector('#lgd_out_pg_pass');
    const elementToHide = document.querySelector('#first_estimate');

    // Add an input event listener to the password input field
    passwordInput.addEventListener('input', function() {
    // Hide the element when the password input changes
        elementToHide.style.display = 'none';
        document.getElementById('initial_estimate').style.display = "block";
    });
}